...
fn test_deposit_withdraw() -> Result<()> {
    ...
    let vault_account = rpc_client.get_account(&vault_pda)?;
    ...
    let balance_after_deposit = rpc_client.get_balance(&user_pubkey)?;
    println!(
    ...
    let balance_after_withdraw = rpc_client.get_balance(&user_pubkey)?;
    println!(
